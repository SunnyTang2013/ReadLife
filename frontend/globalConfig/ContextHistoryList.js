...
const ContextHistoryList = () => {
  const { contextId, contextName } = useParams();
  ...
  function loadHistoryInfo() {
    console.log(`Loading history info by ID: ${contextId}`);
    console.log(`Loading history info by Name: ${contextName}`);

    contextHistoryService.listContextHistory(contextId)
      .then((historyList) => {
    ...
    React.createElement('tr', { key: `row-${historyDetail.id}` },
      React.createElement('td', null,
        React.createElement('td', { className: 'text-nowrap' },
          React.createElement(Link, { to: `/contextHistory/detail/${historyDetail.id}` },
    ...
    React.createElement('div', { className: 'mb-4' },
      React.createElement('nav', null,
        React.createElement('ol', { className: 'breadcrumb' },
          ...
          React.createElement('li', { className: 'breadcrumb-item' },
            React.createElement(Link, { to: `/job-context/detail/${contextId}` }, contextName)
          ),
          <li className="breadcrumb-item active">{`Context History #${contextId}`}</li>
        )
      ),
      <h2 className="display-4">{contextName}</h2>
    ),
    <div>{$historyListTable}</div>
  );
};

export default ContextHistoryList;