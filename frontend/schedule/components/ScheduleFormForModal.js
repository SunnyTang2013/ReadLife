...
const ScheduleFormForModal = ({ schedule, onSave, onCancel, disabled = false }) => {
  ...
  useEffect(() => {
    if (!isEqual(schedule.cronExpression, input.cronExpression)) {
      updateSchedule();
    }
  }, [schedule.cronExpression, input.cronExpression, updateSchedule]);
  ...
  const handleSave = useCallback((event) => {
    ...
    if (!inputData.triggerKeyName) {
      onSave(inputData, true, event);
    } else {
      onSave(inputData, false, event);
    }
  }, [input, onSave]);
    ...
    React.createElement('fieldset', { disabled: disabled || loadingTimezone },
      React.createElement('section', null,
        React.createElement('div', { className: 'form-group' },
          ...
          <input id="schedule-name" className="form-control" type="text" disabled placeholder="Job Group or Job Name, eg.ON_LONDON_RISK_PRIORITY_1" value={input.jobName} onChange={event => onChangeScheduleProperty('jobName'} event) />
        ),
        React.createElement('div', { className: 'form-group' },
...
export default ScheduleFormForModal;